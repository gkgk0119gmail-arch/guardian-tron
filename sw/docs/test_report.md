...
| RTOS | μT-Kernel 3.0 BSP2, 1 ms tick, tasks gate (8) / imu (10) / vision (20) / report (25) / log (30) |
...
|---|---|---|---|---|---|---|---|---|---|
| Context switch | `tk_wup_tsk()` → higher-priority task running (boot benchmark) | < 5.7 µs | 2 000 | 444 ns | 460 ns | 460 ns | 460 ns | **872 ns** | PASS |
| Interrupt → task | USART2 RX interrupt → gatekeeper task running | < 50 µs | 695 600 | 546 ns | 520 ns | 680 ns | 1.10 µs | **9.5 µs** | PASS |
| Command verdict | one Jetson command: CRC, replay, envelope, VESC update, verdict TX queued | < 1 ms | 43 119 | 5.5 µs | 4.4 µs | 8.6 µs | 8.8 µs | **13.4 µs** | PASS |
| **Hazard → brake** | vision (or IMU) raises the hazard → gate task issues the brake frame (all runs: 61 person stops + 5 IMU tilts) | < 100 µs | 66 | 27.8 µs | 30 µs | 33 µs | 33.3 µs | **34 µs** | PASS |
| Frame → decision | camera frame in RAM → NPU inference + decoding + decision | < 100 ms | 12 160 | 29.8 ms | 29.6 ms | 30.6 ms | 30.6 ms | **31.4 ms** | PASS |
| NPU inference | YOLOX-nano 480×480 INT8 | < 50 ms | 12 160 | 28.5 ms | 28.5 ms | 28.5 ms | 28.5 ms | **28.7 ms** | PASS |
| Monitor jitter | \|period − 10 ms\| of the 100 Hz IMU monitor | < 100 µs | 80 932 | 235 ns | 101 ns | 840 ns | 2.0 µs | **11.9 µs** | PASS |
| Monitor step (WCET, observed) | I2C read + Kalman update + decision | < 1 ms | 80 933 | 323 µs | 324 µs | 324 µs | 324 µs | **422 µs** | PASS |
...
| **STM32 μT-Kernel monitor, NPU at full load, Jetson commanding at 100 Hz (run J, n = 80 932)** | **11.9 µs** | |

Worst-case jitter reduction versus Linux under load: **99.6 %** (typical relative-sleep loop, 3 337 µs → 11.9 µs, the more favourable Linux session) and **95 %** (Linux best case, 235 µs → 11.9 µs).
On the Jetson, the host-side round-trip time of a command through the USB-TTL is 10 ms on average and up to 32 ms. The STM32 verdict itself takes at most 13.4 µs.
...
|---|---|---|---|
| Person stop, wheels spinning | person walks in within ≈ 2 m (real person, 6–12 times per run) | stop every time (61 stops across all runs), `detect->gate` 29–34 µs, resume 1.5 s after the person leaves | run A, C, F–J |
...
- **Rare monitor jitter spikes in long runs: fixed.** The four floor runs recorded while filming (121 034 periods) had 20 periods (0.017 %) starting 0.1–0.82 ms late and a 1.14 ms longest step. The cause was the gate task (priority 8) formatting the 10 s metrics report itself. Moving the reports to a `report` task at priority 25 removed it: `run_J` (80 932 periods over 13 min, with the Jetson commanding at 100 Hz) shows **0 periods over 100 µs, max 11.9 µs, longest step 422 µs**. The flashed image and `sw/binaries/` contain the fix.
