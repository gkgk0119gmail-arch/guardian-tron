 ...
 * RX: interrupt -> ring buffer -> event flag.
 * TX: ring buffer drained by the TX-FIFO-not-full interrupt, so a writer never busy-waits
 *     on the wire (a VESC frame is ~2.6 ms at 38400 baud; polling it would stall the
 *     gatekeeper and delay every lower-priority task by as much).
 */
...
#define FLG_RX		(1U << 0)
#define TXBUF_SZ	256			/* power of 2 */
...
typedef struct {
	...
	UB		txbuf[TXBUF_SZ];
	volatile UW	txhead, txtail;
	volatile UW	isr_cyc;		/* DWT stamp of the last RX interrupt that set the flag */
	volatile UW	isr_pending;
	UW		wake_lat;		/* RX interrupt -> reader task running, cycles (0 = none) */
} gt_uart_t;
...
LOCAL void rx_isr(gt_uart_t *u)
{
	...
	if (u->dev->CR1 & USART_CR1_TXEIE_TXFNFIE) {
		while ((u->dev->ISR & USART_ISR_TXE_TXFNF) && u->txtail != u->txhead) {
			u->dev->TDR = u->txbuf[u->txtail];
			u->txtail = (u->txtail + 1) & TXBUF_MASK;
		}
		if (u->txtail == u->txhead) u->dev->CR1 &= ~USART_CR1_TXEIE_TXFNFIE;
	}
	if (!got) return;
	if (!u->isr_pending) { u->isr_cyc = DWT->CYCCNT; u->isr_pending = 1; }
	if (u->flgid > 0) tk_set_flg(u->flgid, FLG_RX);
}

volatile UW gt_irq_usart2;
void USART2_IRQHandler(void) { gt_irq_usart2++; rx_isr(&uarts[GT_UART_JETSON]); }
...
EXPORT ER gt_uart_init(INT unit, UW baud)
{
	...
	d->ICR = 0xFFFFFFFFU;
	/* 8-byte RX FIFO: rides out camera/NPU interrupt bursts without overrun at 115200 */
	d->CR1 = USART_CR1_FIFOEN | USART_CR1_TE | USART_CR1_RE | USART_CR1_RXNEIE_RXFNEIE;
	d->CR1 |= USART_CR1_UE;
...
EXPORT INT gt_uart_read(INT unit, UB *buf, INT len, TMO tmo)
{
	...
	if (u->head == u->tail) {
		...
		if (u->isr_pending) { u->wake_lat = DWT->CYCCNT - u->isr_cyc; u->isr_pending = 0; }
	}
...
EXPORT INT gt_uart_write(INT unit, const UB *buf, INT len)
{
	gt_uart_t *u = &uarts[unit];
	for (INT i = 0; i < len; i++) {
		UW next = (u->txhead + 1) & TXBUF_MASK;
		while (next == u->txtail) {			/* ring full (not expected): let it drain */
			u->dev->CR1 |= USART_CR1_TXEIE_TXFNFIE;
			tk_dly_tsk(1);
		}
		u->txbuf[u->txhead] = buf[i];
		u->txhead = next;
	}
	UINT imask; DI(imask);
	u->dev->CR1 |= USART_CR1_TXEIE_TXFNFIE;		/* ISR moves the bytes into the TX FIFO */
	EI(imask);
	return len;
}
...
/* wake a task blocked in gt_uart_read() without data (returns 0 bytes); used by the
 * vision task to make the gatekeeper react to a hazard immediately */
EXPORT void gt_uart_kick(INT unit) { if (uarts[unit].flgid > 0) tk_set_flg(uarts[unit].flgid, FLG_RX); }
EXPORT UW gt_uart_clock_hz(INT unit) { return uarts[unit].clk_hz; }

/* RX interrupt -> task wake-up latency of the last blocking read, cycles; 0 if none since */
EXPORT UW gt_uart_take_wake_latency(INT unit) { UW l = uarts[unit].wake_lat; uarts[unit].wake_lat = 0; return l; }
