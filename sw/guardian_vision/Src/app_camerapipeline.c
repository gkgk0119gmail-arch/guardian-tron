...
*/
int CameraPipeline_Init(uint32_t *lcd_bg_width, uint32_t *lcd_bg_height, uint32_t *pitch_nn)
{
  int ret;
  ...
  /* Guardian-TRON: a flaky camera ribbon shows up here as an I2C failure; report it
   * instead of asserting so the vision task can power-cycle the sensor and retry */
...
int CameraPipeline_IspUpdate(void)
{
  /* sensor I2C errors (AE/AWB could not program gain/exposure) are not fatal: the next
   * frame still arrives, only the exposure update is skipped */
  return CMW_CAMERA_Run() == CMW_ERROR_NONE ? 0 : -1;
}
...
int CMW_CAMERA_PIPE_FrameEventCallback(uint32_t pipe)
{
  ...
  {
    case DCMIPP_PIPE2 :
      gv_frame_cyc = DWT->CYCCNT;   /* Guardian-TRON: frame-landed timestamp for latency logs */
      cameraFrameReceived++;
      break;
  }
  return 0;
